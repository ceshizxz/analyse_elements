...
def paste_random_nonoverlapping(background, shape, existing_bboxes, side, max_attempts=50, allow_overlap=False):
    ...
    shape_w, shape_h = shape.size
    y_max = int(bg_h * 0.95) - shape_h
    x_max = x_max if side == "right" else int(bg_w * 0.33) - shape_w

    for _ in range(max_attempts):
        x = random.randint(x_min, x_max)
        y = random.randint(y_min, y_max)
        new_bbox = (x, y, x + shape.width, y + shape.height)
        if allow_overlap or all(not bboxes_overlap(new_bbox, b) for b in existing_bboxes):
            background.paste(shape, (x, y), shape)
            return new_bbox
    return None
...
def generate_image_with_shapes(background_path, shape_paths, output_img_path, output_xml_path, side="right"):
    ...
    if count >= 4:
        ...
        overlap_mode = random.choice(["adjacent", "overlap"])

        if overlap_mode == "adjacent":
            ...
            if x_max >= x_min and y_max >= y_min:
                for _ in range(50):
                    x = random.randint(x_min, x_max)
                    y = random.randint(y_min, y_max)
                    background.paste(s1, (x, y), s1)
                    background.paste(s2, (x, y), s2)
                    union_bbox = (x, y, x + shape_w, y + shape_h)
                    if all(not bboxes_overlap(union_bbox, b) for b in bboxes):
                        bboxes.append(union_bbox)
                        names.append("OVERLAPPED_" + n1)
                        break
...
if __name__ == "__main__":
    generate_combined(
        ...
        shape_dir=r"E:\zxz\project\shapes\m1",
        output_img_dir=r"E:\zxz\project\images\generated_regions_b2\images",
        output_xml_dir=r"E:\zxz\project\images\generated_regions_b2\xmls",
        total_count=600
    )