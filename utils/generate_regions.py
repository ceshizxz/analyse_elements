...
def paste_shape_no_overlap(background, shape_path, side, existing_bboxes, scale_factor=10, max_attempts=50):
    ...
    max_scale_w = (bg_width * 0.3) / shape.width
    max_scale_h = (bg_height * 0.8) / shape.height
    safe_scale = min(scale_factor, max_scale_w, max_scale_h, 1.0)
    shape = shape.resize((int(shape.width * safe_scale), int(shape.height * safe_scale)), Image.LANCZOS)
...
if __name__ == "__main__":
    generate_combined(
        ...
        shape_dir=r"E:\zxz\project\shapes",
        output_image_dir=r"E:\zxz\project\images\generated_regions\images",
